...
- [ ] **수행 (EXECUTION) - 초기**
    - [x] 테스트 실행 화면에서 `Pass/Fail` 처리 및 **즉시 결함 보고** 연동.
- [ ] **수행 (EXECUTION) - 회귀 및 패치 (조건부 로직)**
    - [x] **기능 회귀 모드:** 1차 패치 후 기능 테스트 항목에 대한 회귀 테스트 UI 구현.
    - [x] **조건부 활성화(Locking):** 기능 회귀 결과에 따라 보안/성능 테스트 활성/비활성 제어 로직 구현.
    - [x] **파생 결함 마킹:** `3차 이상` 결함은 자동 `isDerived` 마킹.

## Phase 3: Automation & Backend (완벽한 리포트 생성)
- [ ] **Cloud Functions (Report)**
    - [x] **템플릿 관리:** `functions/assets/templates/defect_report_template_fix.xlsx` 업로드.
    - [x] **헤더 치환 로직:** A2, D2 셀 등의 `{Placeholder}` 텍스트 치환 기능 구현.
    - [x] **동적 행 추가:** 템플릿 행의 스타일을 상속받아 결함 데이터 행을 추가하고, 템플릿 행은 삭제.
    - [x] **Data Validation:** 결함정도/발생빈도 컬럼에 드롭다운 메뉴 적용.

## Phase 4: Polish (완성도)
- [ ] **유효성 검사**
    - [x] 4차 리포트 이후에는 결함 수정 상태 변경을 막는 로직(Lock) 추가.
