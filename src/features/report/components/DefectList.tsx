...
export function DefectList({ defects, filter, loading }: DefectListProps) {
  ...
  if (loading) {
    return <div className="rounded-xl border border-surface-200 bg-white p-6 text-sm text-surface-500">불러오는 중...</div>;
  }

  if (filtered.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-surface-200 bg-white p-6 text-sm text-surface-500">
        표시할 결함이 없습니다.
      </div>
    );
  }
