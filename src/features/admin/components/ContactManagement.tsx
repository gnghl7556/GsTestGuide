...
const TABLE_COLUMNS = [
  { label: '역할', className: 'w-36' },
  { label: '이름', className: 'w-28' },
  { label: '연락처', className: 'w-32' },
  { label: '이메일', className: 'w-40' },
  { label: '요청방법', className: 'w-44' },
  { label: '참조 항목' },
...
export function ContactManagement() {
  ...
  const {
    form, setForm,
    editingKey: editingRole, setEditingKey: setEditingRole,
  ...
  return (
    <div className="p-6">
      <AdminPageHeader
        title="담당자 관리"
        description={`역할별 담당자 정보를 관리합니다. 체크리스트에서 해당 역할이 필요한 항목에 자동 표시됩니다. (${roles.length}건)`}
        action={
          <Button size="sm" disabled={busy} onClick={startAdd}>
      ...
      <AdminTable
        ...
        isEmpty={roles.length === 0 && !showAddForm}
      ...
      >
        ...
        {roles.map((r) => {
          ...
          const allSteps = Array.from(new Set([...(roleUsageMap[r.role] ?? []), ...r.linkedSteps]));
          return (
